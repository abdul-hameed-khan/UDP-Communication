...
class Ui_Dialog(object):
    ...
    def send_message_to_receiver(self):
        for i in range(10000):
            message = ''.join(random.choices(string.ascii_uppercase + string.digits, k=1005))+ "&&&&&"+ str(i)
            self.sender_socket.sendto(message.encode(), ('127.0.0.5', 12345))
            self.SenderTextBox.append(f"Sent: {message}")
            QtWidgets.QApplication.processEvents()  # Process events to keep GUI responsive
